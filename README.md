...
Для работы скрипта будем использовать файл журнала *Microsoft IIS* с реального веб сервера, на котором работают пулы приложений программного обеспечения *КриптоПро DSS*. Перед тем, как использовать
...
------------------------------------------------------------------------
```
Для того, чтобы ограничить запуск скрипта одним экземпляром в ***cron***, будем вызывать его с использованием утилиты flock:
```
