...
```
После этого выполняем выбурки из массива с интересующими нас данными и передаём их переменной **smail**:
```
smail=$(
echo "$brd";
echo "Топ 10 активных обращениях к веб серверу за последний час, начиная с $vdate";
...
```
В данном случае используются утилиты ***awk**, **sort**, **uniq** и **head** для структуризации выбираемых строк и ограничения вывода. Так здесь:
```
echo "${array[*]}" | grep -v "::1" | awk '{print $9}' | sort -g | uniq -c -d | sort -gr | head -n 10;
```
мы отфильтровали записи для *localhost* в *IPv6* адресации `grep -v "::1"`, перенправили вывод в **awk**, 
которая выбрала колонку с номером 9 (ip-адрес клиента), далее, строка была перенаправлена на вход утилите **sort**, для группировки ip адресов. 
...
Результатом выполнения данного скрипта будет письмо с отчетом вида:
```
------------------------------------------------------------------------
